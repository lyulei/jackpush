...
@section('content')
    <div style="padding: 15px;">
        ...
        <!-- 代码分类编辑表单 -->
        <form method="post" class="layui-form" action="{{url('CodeType/'.$edit->codeid)}}">
            <input type="hidden" name="_method" value="put">
            {!! csrf_field() !!}
            <label class="layui-inline">代码类型:</label>
            <div class="layui-inline">
                <select name="codetypeid" lay-verify="">
    {!! $list !!}
                </select>
            </div>
            <label class="layui-inline">代码名称:</label>
            <div class="layui-inline">
                <input type="text" name="codename" required  lay-verify="required" placeholder="请输入代码名称" autocomplete="off" class="layui-input" value="{{$edit->codename}}">
            </div>
            ...
            <div class="layui-inline">
                <button class="layui-btn layui-btn-primary">编辑</button> <a href="{{url('CodeType')}}" class="layui-btn">取消</a>
            </div>
        </form>
